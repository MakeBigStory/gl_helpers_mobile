...
//            GL_MEDIUM_FLOAT);
        ...
        unsafe {
            let ptr = get_string(GL_VERSION).unwrap().as_ptr();
            ...
            if major > 2 {
                major = get_integerv(GL_MAJOR_VERSION);
                self.major = major as usize;
                minor = get_integerv(GL_MINOR_VERSION);
        ...
        unsafe {
            ...
            max_textures = get_integerv(GL_MAX_TEXTURE_IMAGE_UNITS);
            self.max_textures = max_textures as usize;

            let mut max_vertex_textures = 0;
            max_vertex_textures = get_integerv(GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS);
            self.max_vertex_textures = max_vertex_textures as usize;

            let mut max_texture_size = 0;
            max_texture_size = get_integerv(GL_MAX_TEXTURE_SIZE);
            self.max_texture_size = max_texture_size as usize;

            let mut max_cube_texture_size = 0;
            max_cube_texture_size = get_integerv(GL_MAX_CUBE_MAP_TEXTURE_SIZE);
            self.max_cube_texture_size = max_cube_texture_size as usize;

            let mut max_render_buffer_size = 0;
            max_render_buffer_size = get_integerv(GL_MAX_RENDERBUFFER_SIZE);
            self.max_render_buffer_size = max_render_buffer_size as usize;

            let mut vs_max_uniforms = 0;
            let mut fs_max_uniforms = 0;
            vs_max_uniforms = get_integerv(GL_MAX_VERTEX_UNIFORM_VECTORS);
            fs_max_uniforms = get_integerv(GL_MAX_FRAGMENT_UNIFORM_VECTORS);
            ...
            max_varyings = get_integerv(GL_MAX_VARYING_VECTORS);
            self.max_varyings = max_varyings as usize * 4;

            let mut max_attributes = 0;
            max_attributes = get_integerv(GL_MAX_VERTEX_ATTRIBS);
...
unsafe fn parse_extenstions(extenstions: &mut Vec<String>, major_version: usize) {
    if major_version > 2 {
        let mut count = 0;
        count = get_integerv(GL_NUM_EXTENSIONS);
